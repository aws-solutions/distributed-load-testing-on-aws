...
exports.handler = async (event) => {
  ...
  try {
    ...
    let runningTasks = await listTasks(listTaskParams);
    ...
    while (runningTasks.length > 0) {
      ...
      for (const task of runningTasksSubset) {
        ...
        promises.push(ecs.stopTask(stopTaskParams).promise().catch((err) => console.error(err)));
      }
      ...
      if (runningTasks.length === 0) runningTasks = await listTasks(listTaskParams);
    }
