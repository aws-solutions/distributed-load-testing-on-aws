...
import { z } from "zod";
import { isLoadTestFramework, type TestType } from "../test-execution.ts";
import { nativeTestFieldsShape, type NativeRunMode } from "./create-test.ts";
import { MAX_TEST_RUNS_PER_DELETE_REQUEST } from "./limits.ts";

/**
 * Zod validation schemas for API request inputs.
 *
 * These are the single shared source of truth for the shapes the API accepts.
 * `api-services` validates incoming requests against them and the CLI derives
 * its request payload types from them (via `z.infer`) so a contract change is
...
function nameSchema(label: string) {
  return z
    ...
    .refine((val: string) => !CONTROL_CHAR.test(val), {
      error: (issue: z.core.$ZodRawIssue) => {
        const codePoint = CONTROL_CHAR.exec(issue.input as string)![0].codePointAt(0)!;
        return `${label} contains a disallowed control character: ${describeControlChar(codePoint)}`;
      },
    });
}
...
const isoDateString = z
  .string()
  .refine((val: string) => !Number.isNaN(Date.parse(val)), "Invalid date format. Expected ISO 8601 format");
...
const validateTimeDuration = (value: string | number, fieldName: string, allowZero: boolean = true): string | true => {
  ...
  const numValue = Number.parseInt(match[1], 10);
  ...
  if (!limit) {
    return `${fieldName}: invalid suffix ${suffix}`;
  }
 ...
 */
export const rampUpSchema = z
  .union([
    z.string().regex(/^\d+[smhd]$/, "ramp-up must be in format: number followed by s, m, h, or d"),
    z.number().int().nonnegative(),
  ])
  .superRefine((val: string | number, ctx: z.RefinementCtx) => {
    const result = validateTimeDuration(val, "ramp-up", true);
    if (result !== true) {
...
export const concurrencySchema = z
  ...
  .transform((val: string | number) => (typeof val === "string" ? Number.parseInt(val, 10) : val))
...
export const testRunsQuerySchema = z
  .object({
    limit: z
      ...
      .optional()
      .transform((val: string | number | undefined) => (typeof val === "string" ? Number.parseInt(val, 10) : val))
      .refine(
...
/**
 * Task count schema (positive integer, string or number form)
 */
export const taskCountSchema = z
  .union([z.number().int().positive(), z.string().regex(/^\d+$/, "taskCount must be a positive integer")])
  .transform((val: string | number) => (typeof val === "string" ? Number.parseInt(val, 10) : val))
  .refine((val: number) => val >= 1, "taskCount must be a positive integer");
...
const testScenarioExecutionSchema = z
  ...
  .passthrough(); // NOSONAR — Allow additional properties for flexibility

/**
 * Request URL schema (valid HTTP/HTTPS URL, max 2048 chars)
 */
export const urlSchema = z
  .string()
  .min(1, "url is required")
  .max(2048, "url must not exceed 2048 characters")
  .url("url must be a valid URL") // NOSONAR
...
const scenarioRequestSchema = z
  .object({
    ...
    method: z.enum(["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"], {
      error: () => "method must be a valid HTTP method (GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS)",
    }),
    headers: z.record(z.string(), z.string()).optional(),
    body: z.string().max(65536, "body must not exceed 65536 characters").optional(),
  })
  .passthrough(); // NOSONAR — Allow additional tool-specific fields
 ...
 */
export const tagsSchema = z.array(z.string()).max(5, "Maximum 5 tags allowed per scenario").optional();

/** Recurrence cadence for scheduled tests. */
export const recurrenceSchema = z.enum(["daily", "weekly", "biweekly", "monthly"]);

/** IANA/DLT schedule timezone string. */
export const scheduleTimezoneSchema = z
  .string()
  .max(64, "scheduleTimezone must not exceed 64 characters");
...
/**
 * Applies cross-field native-mode constraints. Native mode runs the load the
 * uploaded script defines, so the only rule is that it is not valid for simple
 * (scriptless) tests.
 */
function validateNativeRunMode(
  testType: TestType,
  nativeRunMode: NativeRunMode | undefined,
  ctx: z.RefinementCtx
): void {
  if (nativeRunMode && testType === "simple") {
    ctx.addIssue({
      code: "custom",
      path: ["nativeRunMode"],
      message: "nativeRunMode is only supported for jmeter, k6, and locust tests",
    });
  }
}

/**
 * Test name schema (shared name rules, labeled "testName")
 */
export const testNameSchema = nameSchema("testName");

/**
 * Test description schema (3-60000 characters)
 */
export const testDescriptionSchema = z
  .string()
  .min(3, "testDescription must be at least 3 characters")
  .max(60000, "testDescription must not exceed 60000 characters");

/**
 * Healthy threshold schema (integer 0-100)
 */
export const healthyThresholdSchema = z
  .number()
...
export const createTestSchema = z
  .object({
    testId: testIdSchema.optional(),
    testName: testNameSchema,
    testDescription: testDescriptionSchema,
    testType: z.enum(["simple", "jmeter", "locust", "k6"], {
      error: () => "testType must be one of: simple, jmeter, locust, k6",
    }),
    ...
    healthyThreshold: healthyThresholdSchema.optional().default(90),
    // Native-mode fields. Defined in @amzn/dlt-common so the API, the
    // orchestration Lambdas, and the container agree on one contract.
    ...nativeTestFieldsShape,
  })
  .passthrough() // NOSONAR — Allow additional fields for backward compatibility
  .superRefine((data, ctx: z.RefinementCtx) => {
    // Framework tests run a user-supplied script or zip; "none" is only valid for simple tests.
    if (isLoadTestFramework(data.testType) && data.fileType === "none") {
      ctx.addIssue({
        code: "custom",
        path: ["fileType"],
        message: `fileType must be script or zip for ${data.testType} tests`,
      });
    }
    ...
    validateNativeRunMode(data.testType, data.nativeRunMode, ctx);
  });
 ...
 */
export const deleteTestRunsSchema = z
  .array(testRunIdSchema)
  .min(1, "At least one testRunId is required")
  .max(
    MAX_TEST_RUNS_PER_DELETE_REQUEST,
...
export type TestScenarioValidation = z.infer<typeof testScenarioSchema>;
