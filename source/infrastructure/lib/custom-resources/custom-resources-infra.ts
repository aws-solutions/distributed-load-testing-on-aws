...
export class CustomResourceInfraConstruct extends Construct {
  ...
  constructor (scope: Construct, id: string, props: CustomResourceInfraConstructProps) {
    ...
    const scenariosTableArn = Stack.of(this).formatArn({ service: 'dynamodb', region: props.mainStackRegion, resource: 'table', resourceName: props.scenariosTable });
    ...
    if (props.stackType === 'main') {
      customResourceLambda.addToRolePolicy(new PolicyStatement({
        effect: Effect.ALLOW,
