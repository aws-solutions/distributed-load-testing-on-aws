...
export class RegionalPermissionsConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: RegionalPermissionsConstructProps) {
    ...
    const taskArn = Stack.of(this).formatArn({ service: 'ecs', resource: 'task', resourceName: '*', arnFormat: ArnFormat.SLASH_RESOURCE_NAME });
    const taskDefArn = Stack.of(this).formatArn({ service: 'ecs', resource: 'task-definition', resourceName: '*:*' });
    const apiServicesLambdaRoleArn = Stack.of(this).formatArn({ service: 'iam', resource: 'role', resourceName: props.apiServicesLambdaRoleName });
    const lambdaTaskRoleArn = Stack.of(this).formatArn({ service: 'iam', resource: 'role', resourceName: props.taskRunnerRoleName });
    const taskCancelerRoleArn = Stack.of(this).formatArn({ service: 'iam', resource: 'role', resourceName: props.taskCancelerRoleName });
    const taskStatusCheckerRoleArn = Stack.of(this).formatArn({ service: 'iam', resource: 'role', resourceName: props.taskStatusCheckerRoleName });
    const resultsParserRoleNameArn = Stack.of(this).formatArn({ service: 'iam', resource: 'role', resourceName: props.resultsParserRoleName });
