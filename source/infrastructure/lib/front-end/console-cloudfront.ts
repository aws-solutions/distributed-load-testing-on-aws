...
export interface DLTConsoleCloudFrontConstructProps {
  readonly s3LogsBucket: Bucket;
  readonly solutionId: string;
}

export class DLTConsoleCloudFrontConstruct extends Construct {
  public webAppURL: string;
  public consoleBucketArn: string;
  public consoleBucket: IBucket;
  public responseHeadersPolicyId: string;

  constructor(scope: Construct, id: string, props: DLTConsoleCloudFrontConstructProps) {
    super(scope, id);
    ...
    const responseHeadersPolicyProps: ResponseHeadersPolicyProps = {
      ...
      securityHeadersBehavior: {
        ...
        // Note: The exact Cognito domain (e.g. https://dlt-{uuid}.auth.{region}.amazoncognito.com)
        // is added by the UpdateCsp custom resource at deploy time, replacing the need for
        // a wildcard https://*.amazoncognito.com entry.
    ...
    // Extract the ResponseHeadersPolicy ID so the UpdateCsp custom resource can modify the CSP at deploy time
    const rhp = dltS3CloudFrontDist.node.findChild("ResponseHeadersPolicy") as cloudfront.ResponseHeadersPolicy;
    this.responseHeadersPolicyId = rhp.responseHeadersPolicyId;
...
export { DLTConsoleCloudFrontConstruct as DLTConsoleConstruct };
export type { DLTConsoleCloudFrontConstructProps as DLTConsoleConstructProps };
