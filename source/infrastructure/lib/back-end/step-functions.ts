...
import { Chain, Choice, Condition, DISCARD, Fail, Pass, LogLevel, Map as SFMap, StateMachine, Succeed, Wait, WaitTime } from 'aws-cdk-lib/aws-stepfunctions';
...
export class TaskRunnerStepFunctionConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: TaskRunnerStepFunctionConstructProps) {
    ...
    requiresLeader.when(Condition.isNotPresent('$.taskIds'), waitTask);
    ...
    const definition = Chain
      .start(regionConfigsForTest.iterator(checkRunningTests))
      .next(parseResult);
