...
import { RegionalInfrastructureDLTStack, RegionalInfrastructureDLTStackProps } from '../lib/distributed-load-testing-on-aws-regional-stack';

const getProps = (): RegionalInfrastructureDLTStackProps => {
  ...
  const stackType = 'regional';
  const solutionId = 'SO0062';
  const solutionName = SOLUTION_NAME;
  const description = `(${solutionId}-regional) - Distributed Load Testing on AWS testing resources regional deployment. Version ${codeVersion}`;
...
const app = new App();
new RegionalInfrastructureDLTStack(app, 'RegionalInfrastructureDLTStack', {
