...
import { handleDeleteTest } from "../../src/tools/delete-test.js";
import {
...
describe("handleDeleteTest", () => {
  ...
  describe("Successful requests", () => {
    // Verifies DELETE method is sent to /scenarios/{test_id} and response is parsed
    it("should successfully delete a test", async () => {
      const mockResult = { testId: "test-12345", status: "deleted" };
      ...
      const result = await handleDeleteTest(mockHttpClient, apiEndpoint, event);
  ...
  describe("Parameter validation", () => {
    // DeleteTestSchema requires test_id — Zod emits "Required" on missing field
    it("should throw AppError for missing test_id", async () => {
      const event: AgentCoreEvent = {};

      await expect(handleDeleteTest(mockHttpClient, apiEndpoint, event)).rejects.toThrow(AppError);

      try {
        await handleDeleteTest(mockHttpClient, apiEndpoint, event);
      } catch (error) {
  ...
  describe("Error handling", () => {
    ...
    it("should throw AppError with 500 when HTTP client throws", async () => {
      ...
      await expect(handleDeleteTest(mockHttpClient, apiEndpoint, event)).rejects.toThrow(AppError);
      await expect(handleDeleteTest(mockHttpClient, apiEndpoint, event)).rejects.toThrow(
