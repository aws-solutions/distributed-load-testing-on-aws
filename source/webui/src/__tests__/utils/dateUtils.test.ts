...
describe("formatToLocalTime", () => {
  ...
  it("should format date using sourceTimezone", () => {
    const result = formatToLocalTime("2023-12-25 10:30:00", undefined, "America/New_York");
    expect(result).not.toBe("-");
  });

  it("should return '-' for invalid date with sourceTimezone", () => {
    const result = formatToLocalTime("not-a-date", undefined, "America/New_York");
    expect(result).toBe("-");
  });

  it("should apply formatting options with sourceTimezone", () => {
    const options: Intl.DateTimeFormatOptions = {
      year: "numeric",
      month: "short",
      day: "numeric",
    };
    const result = formatToLocalTime("2023-12-25 10:30:00", options, "Europe/London");
    expect(result).toMatch(/Dec.*25.*2023/);
  });
});