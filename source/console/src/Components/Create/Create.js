...
class Create extends React.Component {
  ...
  handleInputChange(event) {
    ...
    // this condition is meant to check if the customer has allowed for enough time interval between
    // successive test runs, as each test run on its own can use up the entire ecs service quota in the account.
    // The interval check should only be done when the customer is actually using this feature in UI, when they select
    // the radio button to schedule a test instead of the option `Run Now`. the state attribute formValues.onSchedule is 1 when
    // the user has selected to the option to schedule the test.
    // The check this.state.formValues.onSchedule === "1" is meant to exclude this condition when the customer chooses to run the test
