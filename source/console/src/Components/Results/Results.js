...
import { Row, Col, Button, Popover, PopoverHeader, PopoverBody, Nav, NavItem, NavLink, TabContent, TabPane, Tooltip, Input, Modal, ModalHeader, ModalBody } from 'reactstrap';
...
class Results extends React.Component {
  ...
  componentDidUpdate = async () => {
    ...
    } else if (this.isUndefined(imageLocation) && !this.isUndefined(this.props.data.metricWidgetImage) && this.props.data.metricWidgetImage !== this.state.metricImage) {
      this.setState({
      ...
      <div>
        <Row>
          ...
          <Col sm="3">
            <div className="result">
              Avg Bandwidth
              <p>{this.calculateBandwidth(data.bytes, testDuration)}</p>
            </div>
          </Col>
        </Row>
        <Row>
          ...
          </Col>
          {
            (
              ...
              <Col className='total-graph-directions'>
                <h3>Graphs are not available for aggregated results across multiple regions.<br /><br />To view graphs you may do one of the following:</h3>
                <ol className='total-graph-directions-list'>
                  <li><b>View graphs by region</b></li>
                  <ul>
                    <li>View the graphs by region using the dropdown above</li>
                  </ul>
                  <li><b>View a total graph with all regions aggregated</b></li>
                  <ul>
                    <li>View the total aggregate graph by using Amazon CloudWatch in the AWS console using the following&nbsp;
                      <Button
                          ...
                          <ModalBody>
                            ...
                            <ol>
                              <li>
                                ...
                                <Button id="total-graph-source" className="total-graph-button" color="link" onClick={() => this.handleCopyClick()} ><i className="icon-small bi bi-clipboard-check-fill" ></i></Button>
                                <Tooltip target="total-graph-source" placement="top" trigger="hover" isOpen={this.state.tooltipOpen} toggle={this.tooltipToggle} >{this.state.tooltipLanguage}</Tooltip>
                                <AceEditor
                                  id="headers"
                                  name="headers"
      ...
      <div>
        <div className="box" id="TestResults">
          <Row>
            <Col xs="9">
              <h2>Test Results</h2>
              <Button id="info" color="link"><i className="bi bi-info-circle-fill" /> Info</Button>
            </Col>
            <Col>
              <Input
                type="select"
              ...
              >
                {
                  this.props.regions.map((key) => (
                    <option
                      value={key}
                      key={key}
                      defaultValue={key === this.props.regions[0]}
                    >
                      {key}
                    </option>
                  )
